ABC-live-build — аббревиатура которая отсылает к первым буквам азбуки, с которых начинают путь все школьники.

Live-build — это официальный набор инструментов проекта Debian, используемый для автоматической сборки образов «живых» систем (Live system). С помощью этого инструмента можно создавать полностью настраиваемые  ISO-образы, образы для USB-накопителей или сетевой загрузки на базе  Debian, Ubuntu и родственных дистрибутивов. 

Основные характеристики и принципы работы

- **Автоматизация:** Live-build преобразует процесс создания образа в набор понятных команд, обеспечивая создание системы «с нуля».
- **Гибкость:** Позволяет настраивать компоненты системы: выбор рабочего стола,  пакетов, локализации, а также добавление собственных файлов и  конфигураций.
- **Использование `chroot`:** Сборка происходит в изолированном окружении, что обеспечивает чистоту итогового образа.
- **Поддержка Live-режимов:** Созданные системы работают без установки на жесткий диск, поддерживают  режим «только чтение» и возможность сохранения изменений (persistence). 

Основные команды (утилиты)

Пакет `live-build` включает несколько основных скриптов, начинающихся с `lb_`: 

1. **`lb config`**: Инициализирует конфигурацию сборки, создавая рабочую директорию и необходимые файлы настроек (в папке `config/`).
2. **`lb build`**: Выполняет непосредственную сборку образа на основе созданной конфигурации.
3. **`lb clean`**: Очищает рабочую директорию от результатов предыдущих сборок и временных файлов. 
...
# 3. Настройка (опционально: добавить пакеты, файлы и т.д.)

# 4. Сборка образа
sudo lb build
```

Live-build является стандартом для создания официальных [Debian Live-образов](https://live-team.pages.debian.net/live-manual/html/live-manual.en.html).  
...
```
У меня вместо обоев - черный фон
```
...
#### Дополним хук командой, которая принудительно выставляет  «Глобальную тему» Breeze, чтобы всё (включая панель) сразу было в одном  стиле?
...
Теперь скрипт для **`02-replace-wallpapers.chroot`** выглядит отлично. Он правильно зачищает систему и подставляет вашу тему `Next`.
...
#### Напишим хук для удаления экранной заставки при начальной загрузке
...
#### Я хочу что бы там было мое название сборок gitcube а не дебиян
...
Хочешь, я покажу, как заменить картинку в этой **Plasma Splash** заставке на твой логотип `gitcube`, чтобы было совсем идеально? 
