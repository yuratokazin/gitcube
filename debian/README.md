### Создание своего дистрибутива на базе Debian 

Это отличный способ получить систему «под себя», или как победить Билла Гейса

Вам понадобится установленный Debian (Astra Linux или «ОСнова»). 

Debootstrap — мощный инструмент для сборка кастомных дистрибутивов. 
Его главное преимущество — возможность с нуля собрать работоспособную систему с точным контролем состава пакетов и конфигурации.

Debian Live Build — современный и надежный инструмент для этого. 

Если они покажутся слишком сложными, есть инструменты попроще:

Cubic (Custom Ubuntu ISO Creator): Графическая утилита (работает и в Debian). Вы просто заходите в «терминал» будущего образа, ставите софт, и она сама собирает ISO.

PinguyBuilder или Systemback: Позволяют сделать слепок (Snapshot) уже настроенной вами живой системы и превратить её в загрузочный образ.

Но я использую **`mmdebstrap`**

Какие ещё инструменты похожи на mmdebstrap?

Существует несколько инструментов, которые похожи на `mmdebstrap` и выполняют схожие задачи по созданию изолированных окружений, chroot-среды или базовых систем. Вот некоторые из них:

## debootstrap

`debootstrap` — один из предшественников `mmdebstrap`. Это инструмент для создания базовой системы Debian в указанном каталоге. Он напрямую работает с `.deb`-пакетами и не использует `apt` для разрешения зависимостей, что может приводить к сложностям при обработке сложных зависимостей. `debootstrap` часто используется для подготовки систем для chroot, контейнеров или установки на новые разделы диска. 

## bdebstrap

`bdebstrap` — обёртка вокруг `mmdebstrap`,  которая позволяет использовать YAML-файлы для конфигурации. Это упрощает хранение настроек и позволяет наслаивать несколько кастомизаций друг на друга, например, для создания вариаций образов. Требует Python ≥ 3 и  некоторые дополнительные модули. manpages.debian.org +2

**Особенности:**

- поддержка многозеркальных репозиториев;
- возможность создания сложных конфигураций через YAML;
- наследует преимущества `mmdebstrap`.

## grml-debootstrap

`grml-debootstrap` — обёртка вокруг `mmdebstrap` (и `debootstrap`), предназначенная для быстрой и простой установки «чистой» системы  Debian. Позволяет настраивать параметры через конфигурационный файл `/etc/debootstrap/config` или командные опции. При запуске без настроек открывается диалоговое окно для базовой конфигурации. 

**Некоторые опции:**

- `--defaultinterfaces` — установка стандартного файла `/etc/network/interfaces`;
- `--non-free` — включение репозитория `non-free`;
- `--nopackages` — пропуск установки пакетов из списка в `/etc/debootstrap/packages`. 

## ti-bdebstrap

`ti-bdebstrap` — набор скриптов, основанных на `bdebstrap`, для создания кастомных образов Debian для платформ Texas Instruments (TI). Включает создание chroot-среды с помощью `bdebstrap`, установку необходимых пакетов TI и сторонних, настройку конфигураций,  сборку U-Boot и т. д. Позволяет создавать полноценные образы Debian для  поддерживаемых платформ TI, которые можно сразу записывать на SD-карту. 

## Сравнение инструментов

| Инструмент         | Особенности                                              | Сценарии использования                            |
| ------------------ | -------------------------------------------------------- | ------------------------------------------------- |
| `debootstrap`      | Работает напрямую с `.deb`-пакетами, не использует `apt` | Создание базовых систем, chroot, контейнеров      |
| `bdebstrap`        | YAML-конфигурация, обёртка над `mmdebstrap`              | Сложные конфигурации, многозеркальные репозитории |
| `grml-debootstrap` | Диалоговое окно для базовой конфигурации                 | Упрощённая установка Debian                       |
| `ti-bdebstrap`     | Специализирован для платформ TI                          | Создание образов Debian для TI-платформ           |

Выбор инструмента зависит от конкретных задач: если нужна простая базовая система — `debootstrap`, если сложная конфигурация с YAML — `bdebstrap`, если специализированное решение для TI — `ti-bdebstrap`.
